...
p1 = ggplot(tab1_all, aes(x = pnmin)) +
  ...
  geom_ribbon(aes(ymin = CH.mu.lb, ymax = CH.mu.ub), alpha = 0.1, fill = "#fdae61", na.rm = TRUE) + 
  geom_ribbon(aes(ymin = CN.mu.lb, ymax = CN.mu.ub), alpha = 0.1, fill = "#2c7bb6", na.rm = TRUE) + 
  ...
  scale_y_continuous(limits = c(-6.5,-2.5), name = "logit-transformed proportion", n.breaks = 10) +
  ...
  scale_colour_manual("Estimates with 95% CI",
                      breaks = c("BN-GLMM","CH model","CN model"),
...
ggsave(filename = "plot-supp.eps", plot = p1, device = cairo_ps, width = 8, height = 5) 
...
taba = data.frame(
  ...
  BN.tau = sprintf("%.3f (%.3f)", lgtP_COPAS_BNGLMM[5,], lgtP_COPAS_BNGLMM[6,]),
  BN.rho = sprintf("%.3f (%.3f)", lgtP_COPAS_BNGLMM[7,], lgtP_COPAS_BNGLMM[8,]),
  CN = sprintf("%.3f (%.3f, %.3f)", tab1_all$CN.mu, tab1_all$CN.mu.lb, tab1_all$CN.mu.ub),
  CN.tau = sprintf("%.3f (%.3f)", lgtP_COPAS1999[5,], lgtP_COPAS1999[6,]),
  CN.rho = sprintf("%.3f (%.3f)", lgtP_COPAS1999[7,], lgtP_COPAS1999[8,]),
  Mc = tab1_all$M.c,
  CH = sprintf("%.3f (%.3f, %.3f)", tab1_all$CH.mu, tab1_all$CH.mu.lb, tab1_all$CH.mu.ub),
  CH.tau = sprintf("%.3f (%.3f)", lgtP_COPAS2000[5,], lgtP_COPAS2000[6,]),
  CH.rho = sprintf("%.3f (%.3f)", lgtP_COPAS2000[7,], lgtP_COPAS2000[8,])
)
...
kbl(taba1, 
    ...
    label = "tab")%>% 
  add_header_above(c("", "","Proposed method (BN-GLMM)" = 3))%>%
