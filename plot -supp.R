...
svt <- T
## LOAD R DATA
load("Rdata/example-supp-bias1.RData")
...
p1 <- ggplot(tab1_all, aes(x = pnmax)) +
  ...
  scale_x_reverse(n.breaks = 10, name="Probability of publishing studies with largest sample size / smallest SE") +
...
## LOAD R DATA
load("Rdata/example-supp-bias2.RData")
...
if(svp) ggsave(filename = "plot-supp.eps", plot = plot, device = cairo_ps, width = 12, height = 6) 
