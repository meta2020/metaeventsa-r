...
data <- read.csv("niel-weise21.csv")[sample(5),]
...
lnOR_hn <- HN_GLMM(
  ...
  mu.upper = 3, tau.upper = 3, integ.limit = 20, 
  init.vals = c(-0.1,0.1))
...
yi <- log((data_cc$y1/data_cc$c1)/(data_cc$y0/data_cc$c0))
vi <- 1/data_cc$y1 + 1/data_cc$c1 + 1/data_cc$y0 + 1/data_cc$c0

lnOR_nn <- NN_LMM(
  yi=yi, vi=vi, 
  mu.upper = 3, tau.upper = 3,
  init.vals = c(-0.1,0.1))
...
lnOR_copas_HNGLMM <- vapply(
  p_sa, 
  function(p) {
    mod <- copas_HNGLMM(y0=y0, y1=y1, n0=n0, n1=n1, Pnmax = 0.99, Pnmin = p, 
...
lnOR_copas_BNGLMM <- vapply(
  p_sa, 
  function(p) {
    mod <- copas_BNGLMM(y0=y0, y1=y1, n0=n0, n1=n1, Pnmax = 0.99, Pnmin = p, 
                        rho.init = 0.1, mu.upper = 3, tau.upper = 3, integ.limit = 20,
